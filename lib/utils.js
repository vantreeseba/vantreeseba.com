  ...
  return Object.entries(data).map(([key, value]) => {
    ...
    if(_.isArray(value) && _.isString(value[0])) {
      const values = value.map(x => {
        return <div key={`${key}-value`} className={styles.value}>{x}</div>;
      })
      value = <div key={`${key}-value`} className={styles.value}>{values}</div>;
    } else {
      value = <div key={`${key}-value`} className={styles.value}>{value}</div>;
    }
    return (
      <>
        <div key={`${key}`} className={styles.key}>{title}</div>
        {value}
      </>
    );
  });
